...
impl LiteParse {
    fn new(kwargs: Option<RHash>) -> Result<Self, Error> {
        ...
        if let Some(ref kwargs) = kwargs {
            ...
            if let Some(v) = kwarg::<bool>(kwargs, "ocr_enabled") { cfg.ocr_enabled = v; }
            ...
            if let Some(v) = kwarg::<bool>(kwargs, "preserve_very_small_text") { cfg.preserve_very_small_text = v; }
            if let Some(v) = kwarg::<String>(kwargs, "password") { cfg.password = Some(v); }
            if let Some(v) = kwarg::<bool>(kwargs, "quiet") { cfg.quiet = v; }
            ...
            if let Some(v) = kwarg::<bool>(kwargs, "extract_links") { cfg.extract_links = v; }
        }
